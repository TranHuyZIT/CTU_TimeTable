...
export default function CourseDetailModal({
  ...
  useEffect(() => {
    if (course.key) {
      dispatch(courseListSlice.actions.clearCourse());

      const yearQuery = yearSemester.year.replace("-", "");
      axios
        .get(
          `https://api.tools.w5team.com//courses/key/${course.key}?y=${yearQuery}&n=${yearSemester.semester}`
        )
        .then((res) => {
          setGroups(res.data);
          setLoading(false);
                        ...
                        >
                          {groups.map((group) => (
                            <MenuItem
                              key={group.class}
                              value={group}
                            >{`Nhóm ${group.id}, sĩ số còn lại: ${group.available}`}</MenuItem>
                          ))}
                        </Select>
                      </FormControl>
                    </>
